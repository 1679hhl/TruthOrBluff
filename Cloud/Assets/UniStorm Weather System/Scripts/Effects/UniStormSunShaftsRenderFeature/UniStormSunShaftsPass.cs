...
public class UniStormSunShaftsPass : ScriptableRenderPass
{
    ...
    public Transform SunTransform
    {
        get
        {
            ...
            {
                    Light[] lights = UnityEngine.Object.FindObjectsByType<Light>(FindObjectsSortMode.None).Where(l => l.type == LightType.Directional).ToArray();
                if (lights.Length > 0)
                {
    ...
    public UniStormSunShaftsPass(string tag)
    {
        ...
        [Obsolete("Configure(CommandBuffer, RenderTextureDescriptor) 已过时，请使用 Render Graph API。")]
    ...
    /// <inheritdoc/>
        [Obsolete("Execute(ScriptableRenderContext, ref RenderingData) 已过时，请使用 Render Graph API。")]
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        // XRSettings API 兼容性处理
#if UNITY_2022_1_OR_NEWER
        var stereoRenderingMode = UnityEngine.XR.XRSettings.stereoRenderingMode;
#else
        var stereoRenderingMode = XRSettings.stereoRenderingMode;
#endif
        if (stereoRenderingMode == UnityEngine.XR.XRSettings.StereoRenderingMode.SinglePassInstanced
            && cameraRenderTextureDescriptor.volumeDepth == 2)
        {
            cmd.EnableShaderKeyword("STEREO_INSTANCING_ON");
        }

        if (stereoRenderingMode == UnityEngine.XR.XRSettings.StereoRenderingMode.MultiPass
        ...
        for (int it2 = 0; it2 < settings.radialBlurIterations; it2++)
        {
            ...
            RenderTexture.ReleaseTemporary(lrColorB);
                ...
                // Render Graph API 推荐使用 RTHandle 进行目标设置，兼容性处理
                // ConfigureTarget(tmpBufferRTId); // 已过时，建议迁移到 RTHandle
            sunShaftsMaterial.SetVector("_BlurRadius4", new Vector4(ofs, ofs, 0.0f, 0.0f));
        }
